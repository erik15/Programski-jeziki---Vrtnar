...
public class SajenjeArrayAdapter extends ArrayAdapter<Sajenja> { //Step 4.8 POPRAVI Stevec -> Rezultati
	LayoutInflater mInflater;
	public SajenjeArrayAdapter(Context context, int textViewResourceId, List<Sajenja> objects) { //Step 4.8 POPRAVI Stevec ->Rezultati
		super(context, textViewResourceId,objects);
	    mInflater = LayoutInflater.from(context);
	}
	@Override
	public View getView(int position, View convertView, ViewGroup parent) {
		Sajenja tmp = getItem(position);
		ViewHolder holder;
		// When convertView is not null, we can reuse it directly, there is no need
		// to reinflate it. We only inflate a new View when the convertView supplied
		// by ListView is null.
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.sajenje_layout, null);
